...
public class AuthzPolicyCreatePresenterTest extends PolicyAdminUIGWTTestBase {

	AuthzPolicyCreatePresenter authzPolicyCreatePresenter = null;
	HandlerManager eventBus = null;
	Map<SupportedService, PolicyAdminUIService> serviceMap = null;
	AuthzPolicyCreateView view = null;
	...
	public void setUp() {
		...
		view = createMock(AuthzPolicyCreateView.class);
	}
	...
	public void tearDown() {
		serviceMap = null;
		eventBus = null;
		authzPolicyCreatePresenter = null;
	}

	@Test
	public void testFetchSubjects() {
		authzPolicyCreatePresenter = new AuthzPolicyCreatePresenter(this.eventBus,
				view, serviceMap);
		authzPolicyCreatePresenter.service = (PolicyQueryService) serviceMap
				.get(SupportedService.POLICY_QUERY_SERVICE);
		replay(view);

		PolicyType.init(authzPolicyCreatePresenter.service,
		...
		ResourceType.init(authzPolicyCreatePresenter.service,
				new AsyncCallback<List<String>>() {
					public void onFailure(final Throwable arg) {
						// do nothing
					}
		...
		SubjectType.init(authzPolicyCreatePresenter.service,
				new AsyncCallback<List<String>>() {
					public void onFailure(final Throwable arg) {
						// do nothing
					}
		...
		GwtReflectionUtils.callPrivateMethod(authzPolicyCreatePresenter,
				"fetchSubjects");
		assertNotNull(authzPolicyCreatePresenter.allSubjects);
		assertEquals(DummyPolicyQueryServiceImpl.tmpSubjects.size(),
				authzPolicyCreatePresenter.allSubjects.size());
	...
	public void testFetchResources() {
		...
		GwtReflectionUtils.callPrivateMethod(authzPolicyCreatePresenter,
				"fetchResources");
		assertNotNull(authzPolicyCreatePresenter.availableResourcesByType);
		assertEquals(DummyPolicyQueryServiceImpl.tmpResources.size(),
				authzPolicyCreatePresenter.availableResourcesByType.size());
