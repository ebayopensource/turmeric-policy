...
public class AuthzPolicyEditPresenterTest extends PolicyAdminUIGWTTestBase {

	AUTHZPolicyEditPresenter AUTHZPolicyEditPresenter = null;
	HandlerManager eventBus = null;
	Map<SupportedService, PolicyAdminUIService> serviceMap = null;
	AUTHZPolicyEditView view = null;
	...
	public void setUp() {
		...
		view = createMock(AUTHZPolicyEditView.class);
	}
	...
	public void tearDown() {
		serviceMap = null;
		eventBus = null;
		AUTHZPolicyEditPresenter = null;
	}

	@Test
	public void testFetchSubjects() {
		AUTHZPolicyEditPresenter = new AUTHZPolicyEditPresenter(this.eventBus,
				view, serviceMap);
		AUTHZPolicyEditPresenter.service = (PolicyQueryService) serviceMap
				.get(SupportedService.POLICY_QUERY_SERVICE);
		replay(view);

		PolicyType.init(AUTHZPolicyEditPresenter.service,
		...
		ResourceType.init(AUTHZPolicyEditPresenter.service,
				new AsyncCallback<List<String>>() {
					public void onFailure(final Throwable arg) {
						// do nothing
					}
		...
		SubjectType.init(AUTHZPolicyEditPresenter.service,
				new AsyncCallback<List<String>>() {
					public void onFailure(final Throwable arg) {
						// do nothing
					}
		...
		GwtReflectionUtils.callPrivateMethod(AUTHZPolicyEditPresenter,
				"fetchSubjects");
		assertNotNull(AUTHZPolicyEditPresenter.allSubjects);
		assertEquals(DummyPolicyQueryServiceImpl.tmpSubjects.size(),
				AUTHZPolicyEditPresenter.allSubjects.size());
	...
	public void testFetchResources() {
		...
		GwtReflectionUtils.callPrivateMethod(AUTHZPolicyEditPresenter,
				"fetchResources");
		assertNotNull(AUTHZPolicyEditPresenter.availableResourcesByType);
		assertEquals(DummyPolicyQueryServiceImpl.tmpResources.size(),
				AUTHZPolicyEditPresenter.availableResourcesByType.size());
