...
public class BLPolicyCreatePresenterTest extends PolicyAdminUIGWTTestBase {

	BLPolicyCreatePresenter blPolicyCreatePresenter = null;
	HandlerManager eventBus = null;
	Map<SupportedService, PolicyAdminUIService> serviceMap = null;
	BLPolicyCreateView view = null;
	...
	public void setUp() {
		...
		view = createMock(BLPolicyCreateView.class);
	}
	...
	public void tearDown() {
		serviceMap = null;
		eventBus = null;
		blPolicyCreatePresenter = null;
	}

	@Test
	public void testFetchSubjects() {
		blPolicyCreatePresenter = new BLPolicyCreatePresenter(this.eventBus,
				view, serviceMap);
		blPolicyCreatePresenter.service = (PolicyQueryService) serviceMap
				.get(SupportedService.POLICY_QUERY_SERVICE);
		replay(view);

		PolicyType.init(blPolicyCreatePresenter.service,
		...
		ResourceType.init(blPolicyCreatePresenter.service,
				new AsyncCallback<List<String>>() {
					public void onFailure(final Throwable arg) {
						// do nothing
					}
		...
		SubjectType.init(blPolicyCreatePresenter.service,
				new AsyncCallback<List<String>>() {
					public void onFailure(final Throwable arg) {
						// do nothing
					}
		...
		GwtReflectionUtils.callPrivateMethod(blPolicyCreatePresenter,
				"fetchSubjects");
		assertNotNull(blPolicyCreatePresenter.allSubjects);
		assertEquals(DummyPolicyQueryServiceImpl.tmpSubjects.size(),
				blPolicyCreatePresenter.allSubjects.size());
	...
	public void testFetchResources() {
		...
		GwtReflectionUtils.callPrivateMethod(blPolicyCreatePresenter,
				"fetchResources");
		assertNotNull(blPolicyCreatePresenter.availableResourcesByType);
		assertEquals(DummyPolicyQueryServiceImpl.tmpResources.size(),
				blPolicyCreatePresenter.availableResourcesByType.size());
