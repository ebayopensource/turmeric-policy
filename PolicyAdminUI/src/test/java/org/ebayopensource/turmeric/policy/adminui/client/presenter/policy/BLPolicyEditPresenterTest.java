...
public class BLPolicyEditPresenterTest extends PolicyAdminUIGWTTestBase {

	BLPolicyEditPresenter blPolicyEditPresenter = null;
	HandlerManager eventBus = null;
	Map<SupportedService, PolicyAdminUIService> serviceMap = null;
	BLPolicyEditView view = null;
	...
	public void setUp() {
		...
		view = createMock(BLPolicyEditView.class);
	}
	...
	public void tearDown() {
		serviceMap = null;
		eventBus = null;
		blPolicyEditPresenter = null;
	}

	@Test
	public void testFetchSubjects() {
		blPolicyEditPresenter = new BLPolicyEditPresenter(this.eventBus,
				view, serviceMap);
		blPolicyEditPresenter.service = (PolicyQueryService) serviceMap
				.get(SupportedService.POLICY_QUERY_SERVICE);
		replay(view);

		PolicyType.init(blPolicyEditPresenter.service,
		...
		ResourceType.init(blPolicyEditPresenter.service,
				new AsyncCallback<List<String>>() {
					public void onFailure(final Throwable arg) {
						// do nothing
					}
		...
		SubjectType.init(blPolicyEditPresenter.service,
				new AsyncCallback<List<String>>() {
					public void onFailure(final Throwable arg) {
						// do nothing
					}
		...
		GwtReflectionUtils.callPrivateMethod(blPolicyEditPresenter,
				"fetchSubjects");
		assertNotNull(blPolicyEditPresenter.allSubjects);
		assertEquals(DummyPolicyQueryServiceImpl.tmpSubjects.size(),
				blPolicyEditPresenter.allSubjects.size());
	...
	public void testFetchResources() {
		...
		GwtReflectionUtils.callPrivateMethod(blPolicyEditPresenter,
				"fetchResources");
		assertNotNull(blPolicyEditPresenter.availableResourcesByType);
		assertEquals(DummyPolicyQueryServiceImpl.tmpResources.size(),
				blPolicyEditPresenter.availableResourcesByType.size());
