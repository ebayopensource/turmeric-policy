...
public class PolicySummaryPresenter extends AbstractGenericPresenter {
    ...
    private void removePolicy(List<GenericPolicy> policies, PolicyKey key) {
        ...
        while (itor.hasNext()) {
            ...
            if (pol.getName().equals(key.getName()) && pol.getType().equals(key.getType()))
                itor.remove();
        }
    }
    ...
    private List<GenericPolicy> copyToWriteable(Collection<GenericPolicy> policies) {
        ...
        for (GenericPolicy p : policies) {
            if ((!p.getName().startsWith("Admin_Policy_") && !p.getName().startsWith("PolicyService") && !p.getName()
                            .startsWith("SuperPolicy")) || AppUser.getUser().isAdminUser()) {
    ...
    private void fetchResourcesByType(String type) {
        ...
        service.getResources(Collections.singletonList(key), new AsyncCallback<GetResourcesResponse>() {
            ...
            public void onFailure(Throwable arg) {
                if (arg.getLocalizedMessage().contains("500")) {
                    view.error(PolicyAdminUIUtil.messages.serverError(PolicyAdminUIUtil.policyAdminConstants
                                    .genericErrorMessage()));
                }
                else {
                    view.error(PolicyAdminUIUtil.messages.serverError(arg.getLocalizedMessage()));
            ...
            public void onSuccess(GetResourcesResponse result) {
                PolicySummaryPresenter.this.resources = new ArrayList<Resource>(result.getResources());
    ...
    private void fetchPoliciesBySubjectGroupName(String name, String type) {
        ...
        QueryCondition condition = new QueryCondition();
        condition.addQuery(new QueryCondition.Query(ActivePoliciesOnlyValue.FALSE));

        service.findPolicies(null, polKeys, null, null, null, Collections.singletonList(key), null, condition,
                        new AsyncCallback<GetPoliciesResponse>() {
    ...
    private void fetchPoliciesBySubject(String subjectName, String subjectType) {
        ...
        QueryCondition condition = new QueryCondition();
        condition.addQuery(new QueryCondition.Query(ActivePoliciesOnlyValue.FALSE));
        service.findPolicies(null, polKeys, null, null, Collections.singletonList(subjectKey), null, null, condition,
                        new AsyncCallback<GetPoliciesResponse>() {
    ...
    private void fetchPoliciesByResource(final String resourceType, final String resourceName,
        ...
        QueryCondition condition = new QueryCondition();
        condition.addQuery(new QueryCondition.Query(ActivePoliciesOnlyValue.FALSE));
        service.findPolicies(null, polKeys, Collections.singletonList(resKey), opKeys, null, null, null, condition,
                        new AsyncCallback<GetPoliciesResponse>() {
    ...
    private void fetchAccess(final GenericPolicy policy) {
        ...
        fetchAccess(UserAction.POLICY_DELETE, policy, new AsyncCallback<Boolean>() {
            public void onFailure(Throwable arg) {
                if (arg.getLocalizedMessage().contains("500")) {
                    view.error(PolicyAdminUIUtil.messages.serverError(PolicyAdminUIUtil.policyAdminConstants
                                    .genericErrorMessage()));
        ...
        fetchAccess(UserAction.POLICY_EDIT, policy, new AsyncCallback<Boolean>() {
            public void onFailure(Throwable arg) {
                if (arg.getLocalizedMessage().contains("500")) {
                    view.error(PolicyAdminUIUtil.messages.serverError(PolicyAdminUIUtil.policyAdminConstants
                                    .genericErrorMessage()));
    ...
    private void fetchSuperAdminAccess(final UserAction action, final GenericPolicy policy,
        ...
        enforcementService.verify(opKey, policyTypes, credentials, subjectTypes, null, accessControlObject, null,
                        new AsyncCallback<VerifyAccessResponse>() {
