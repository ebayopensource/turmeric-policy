	...
	private class ContentView extends AbstractGenericView implements Display {
		...
		public void initialize() {
			...
			Column<SubjectGroup, SubjectGroup> viewColumn = new Column<SubjectGroup, SubjectGroup>(
					new CustomPermissionCheckboxCell<SubjectGroup, Boolean>(
							UserAction.SUBJECT_GROUP_VIEW, pendingActions,
							permittedActions)) {
					...
					.setFieldUpdater(new FieldUpdater<SubjectGroup, SubjectGroup>() {
						...
						public void update(int arg0, SubjectGroup arg1,
							...
							if (pendingActions.keySet().contains(arg1)) {
								...
								pendingActions.put(arg1,
										UserAction.SUBJECT_GROUP_VIEW);
			...
			Column<SubjectGroup, SubjectGroup> deleteColumn = new Column<SubjectGroup, SubjectGroup>(
					new CustomPermissionCheckboxCell<SubjectGroup, Boolean>(
							UserAction.SUBJECT_GROUP_DELETE, pendingActions,
							permittedActions)) {
				public SubjectGroup getValue(SubjectGroup group) {
					return group;
				}
			...
			deleteColumn
					.setFieldUpdater(new FieldUpdater<SubjectGroup, SubjectGroup>() {
						public void update(int index, SubjectGroup group,
								SubjectGroup arg2) {
							...
							if (pendingActions.keySet().contains(group)) {
								...
								// Called when the user clicks on a checkbox.
								// Many groups can be selected for delete at the
								// same time,
								// but any other pending ops must be cancelled
								Iterator<Map.Entry<SubjectGroup, UserAction>> itor = pendingActions
										.entrySet().iterator();
								while (itor.hasNext()) {
									Map.Entry<SubjectGroup, UserAction> entry = itor
											.next();
									if (!entry.getValue().equals(
											UserAction.SUBJECT_GROUP_DELETE))
										itor.remove();
								}

								updateButton(UserAction.SUBJECT_GROUP_DELETE);
								pendingActions.put(group,
										UserAction.SUBJECT_GROUP_DELETE);
							}
							cellTable.redraw();
						}
			...
			cellTable.addColumn(groupNameCol,
			...
			// subject description
			Column<SubjectGroup, String> descCol = new Column<SubjectGroup, String>(
					new DescriptionCell()) {
			...
			cellTable.addColumn(descCol,
					PolicyAdminUIUtil.policyAdminConstants.description());
