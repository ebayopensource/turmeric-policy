...
<xsl:stylesheet version="2.0" 
	...
	<xsl:param name="use-namespaces" as="xs:boolean" select="true()"/>
	...
	<xsl:function name="json:create-children">
		...
		<xsl:choose>
			<xsl:when test="exists($node/child::text()) and count($node/child::node()) eq 1">
				<xsl:choose>
					<xsl:when test="(count($node/namespace::*) gt 0 and not($use-namespaces)) or count($node/@*[not(../@json:force-array) or count(.|../@json:force-array)=2]) gt 0">
						<json:object>
							<!-- <xsl:copy-of select="json:create-namespaces($node)"/> -->
							<xsl:copy-of select="json:create-attributes($node)"/>
							<json:member>
								<json:name>__value__</json:name>
				...
				<json:object>
					...
					<xsl:for-each-group select="$node/child::node()" group-adjacent="local-name()">
						<xsl:choose>
							...
							<xsl:otherwise>
								<json:member>
									<json:name><xsl:value-of select="if($use-namespaces) then translate(current-group()[1]/name(), ':','.') else current-group()[1]/local-name()"/></json:name>
									<json:value>
										<json:array>
	...
	<xsl:function name="json:create-string" as="node()">
		...
		<xsl:choose>
			<xsl:when test="$use-namespaces">
				<json:name><xsl:value-of select="translate($node/name(), ':','.')"/></json:name>	
			</xsl:when>
			<xsl:otherwise>
...
<!--  	<xsl:function name="json:create-namespaces" as="node()*">
		...
		<xsl:if test="$use-namespaces">
			<xsl:if test="count($node/namespace::*) gt 0">
				<json:member>
					<json:name><xsl:if test="$use-badgerfish or $use-rabbitfish">@</xsl:if></json:name>
					<json:value>
						<json:object>
